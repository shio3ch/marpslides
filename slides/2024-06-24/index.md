...
- バッチのコアロジックとAPIに関心事を分離
  - 既存のアーキテクチャ（namespaceの構造）が弊害
...
- テストするためにはバッチ処理全体をコンテキストとして捉えなければならない
  - 外部APIとの接続ロジックのテストに集中したくても、前後のロジックを通すための準備がテストの弊害となる
  - 異常系のテストはライブラリで無理やりモック化するしかなくなる
...
- 新機能
  - 最初からテストコードを書こう
- 既存機能
  - 仕様変更のタイミングで、徐々にテストコードを適用する
  - ピンニングテスト（通しテスト）は苦痛なら書かない
    - **最初から書くことが困難なテストは誰もメンテ出来ない**
  - モデルの中の小さな関心事（=メソッド）を見つけ出してテストを書く
    - テストのために private → public に可視性を変更するのもあり
      （あくまで今のモデルに対してのみ。良いクラス設計であれば可視性を変える必要はない）
...
- すべてのコードにテストを適用する必要はない
  - 書きづらい=メンテしづらいなので逆効果になる
- テストライブラリで無理やりモック化しない
  - テストコードがプロダクトコードの中身を知ることになる（=変更に弱い）
  - ロジック仕様の変わらない変更でも影響が出る場合がある
  - モックライブラリを使いたくなった場合、プロダクトコード自体に問題がある
